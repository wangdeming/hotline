        ...
        <div class="rightDiv">
          ...
          <div class="rightDivTwo">
				...
				<div class="rightContent">
					<div class="valueDiv">
						<span class="valueItem gridManValueOne">0%</span>
						<span class="valueItem gridManValueTwo">0%</span>
					</div>
					<div class="chartDiv">
						<div class="gridmanDiv colorOne"></div>
						<div class="noGridmanDiv colorTwo"></div>
					</div>
					<div class="nameDiv">
						<span class="colorSpan colorOne"></span>
						<span class="nameItem">网格员</span>
						<span class="colorSpan colorTwo"></span>
						<span class="nameItem">非网格员</span>
					</div>
				</div>
		  </div>
          <div class="rightDivThree">
            ...
            <div class="rightContent" id="publicInfoChart">
              <div class="valueDiv">
                <span class="valueItem publishInfoValueOne">0%</span>
                <span class="valueItem publishInfoValueTwo">0%</span>
              </div>
              <div class="chartDiv">
...
//			DateSelect
		...
		methods: {
			...
			initIndex() {
				...
				this._getHotEventCrowdGridDistribution();
			},
//			onChangeDate(state) {
...
//			},
			...
			_getHotEventCrowdGridDistribution() {
				api.get_hotEventCrowdGridDistribution({
					hotEventId: this.params.eventId,
					startTime: this.startTime,
					endTime: this.endTime,
					timeScope: this.timeScope
				}).then(res => {
					var data = res.data; //0:非网格员，1:网格员
					var gridNum = 0;
					var nogridNum = 0;
					if(data.length == 0) {
//						$(".gridmanDiv").text(0);
//						$(".noGridmanDiv").text(0);
						$(".gridmanDiv").css("width", "50%");
						$(".gridManValueOne").text("0%").css("left", "calc(25% - 30px)");
						$(".noGridmanDiv").css("width", "50%");
						$(".gridManValueTwo").text("0%").css("left", "calc(75% - 30px)");
					} else {
						for(var i = 0; i < data.length; i++) {
							if(data[i].name == 1) {
								gridNum = data[i].value;
//								$(".gridmanDiv").text(gridNum);
							}
							if(data[i].name == 0) {
								nogridNum = data[i].value;
//								$(".noGridmanDiv").text(nogridNum);
							}
						}
						var amount = gridNum + nogridNum;
						var gridPer = (gridNum * 100 / parseFloat(amount)).toFixed(1) + '%';
						$(".gridmanDiv").css("width", gridPer);
						var gridPoint = gridNum * 50 / parseFloat(amount);
						$(".gridManValueOne").text(gridPer).css("left", "calc(" + gridPoint + "% - 30px)");
						var nogridPer = (nogridNum * 100 / parseFloat(amount)).toFixed(1) + '%';
						$(".noGridmanDiv").css("width", nogridPer);
						var nogridPoint = (gridNum * 100 + nogridNum * 50) / parseFloat(amount);
						$(".gridManValueTwo").text(nogridPer).css("left", "calc(" + nogridPoint + "% - 30px)");
					}
				});
			},
			_getQuerySexDistribution() {
				api.get_hotEventCrowdQuerySexDistribution({
					hotEventId: this.params.eventId,
					startTime: this.startTime,
					endTime: this.endTime,
					timeScope: this.timeScope
...
//								$(".womenDiv").text(womenNum);
			...
			_getQueryIsPublic() {
				api.get_hotEventCrowdQueryIsPublic({
					hotEventId: this.params.eventId,
					startTime: this.startTime,
					endTime: this.endTime,
					timeScope: this.timeScope
...
//								$(".noPublishInfo").text(noPublishNum);
			...
			_getTimeDistribution() {
				api.get_hotEventCrowdTimeDistribution({
					hotEventId: this.params.eventId,
					startTime: this.startTime,
					endTime: this.endTime,
					timeScope: this.timeScope
			...
			_getJobDistribution() {
				api.get_hotEventCrowdJobDistribution({
					hotEventId: this.params.eventId,
					startTime: this.startTime,
					endTime: this.endTime,
					timeScope: this.timeScope
			...
			_getDistrictDistribution() {
				api.get_hotEventCrowdDistrictDistribution({
					hotEventId: this.params.eventId,
					startTime: this.startTime,
					endTime: this.endTime,
					timeScope: this.timeScope
...
<style scoped lang="stylus">
	...
	.rightDivOne,.rightDivTwo,.rightDivThree{
		width: 100%;
		height: 189px;
		margin-bottom: 10px;
	}
